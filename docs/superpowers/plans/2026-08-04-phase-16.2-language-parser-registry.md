...
### Task 9: Docs, ROADMAP, and release 3.11.0
...
- [ ] **Step 2: CHANGELOG** — include the language parser registry under `## 3.11.0`: Python indexing (char-accurate scanner, full declaration spans, relative + repo-root absolute resolution, function-local imports); additive `language` node field (`schema_version` unchanged at `1`); compiler/expansion routed through the registry; Python Markdown fences; `.venv`/`__pycache__` graph ignore; activation of Phase 16.1 exclusion enforcement for `.py`/`.pyi`; note that Python-containing repos report `stale` once after upgrade.
...
git commit -m "docs: document language parser registry for release 3.11.0"
```
...
- Docs/ROADMAP/release 3.11.0 → Task 9. ✓
