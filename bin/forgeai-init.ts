...
import { runObservedCheck } from './lib/run-events.js';
...
else if (checkGit) runObservedCheck('git', runCheckGit);
else if (checkSessions) runObservedCheck('sessions', runCheckSessions);
else if (checkLifecycle) runObservedCheck('lifecycle', runCheckLifecycle);
else if (checkCodeGraph) runObservedCheck('codegraph', () => runCheckCodeGraph({ strict }));
else if (refreshCodeGraph) runRefreshCodeGraph();
else if (checkProfile) runObservedCheck('profile', runCheckProfile);
else if (checkReview) runObservedCheck('review', runCheckReview);
else if (checkSecurity) runObservedCheck('security', runCheckSecurity);
else if (checkMemory) runObservedCheck('memory', runCheckMemory);
else if (checkAll) runObservedCheck('all', runCheckAll);
else if (check) runObservedCheck('harness', runCheck);
...
else if (checkEvaluation) runObservedCheck('evaluation', runCheckEvaluation);
